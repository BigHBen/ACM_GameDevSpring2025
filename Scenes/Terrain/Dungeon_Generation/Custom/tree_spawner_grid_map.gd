	...
	for x in range(border_start, border_end): # If not centered
		for z in range(border_start, border_end):
			...
			if dun_gen:
				...
				# Instead of placing 'walls' in empty cells
				# Place 'trees' on top of non empty grass cells
				if grass_gridmap.get_cell_item(pos) != -1: 
					var tree_mesh = tree_items.pick_random()
					self.set_cell_item(pos, tree_mesh)
					positions.append(pos)
				#if source_gridmap.get_cell_item(pos) == -1: # if empty tile
					#self.set_cell_item(pos, 0)
					#positions.append(pos)
